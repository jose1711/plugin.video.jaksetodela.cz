...
class JaksetodelaContentProvider(ContentProvider):
    ...
    def __init__(self, username=None, password=None, filter=None):
        ContentProvider.__init__(
            self, 'jaksetodela.cz', 'http://www.jaksetodela.cz/', username, password, filter)
    ...
    def search(self, keyword):
        return self.catl3(util.parse_html(self._url('search/?search_id=' + keyword +
                                                    '&search_type=search_videos&submit=Hledej')))
    ...
    def catl2(self, page):
        ...
        result = []
        try:
            navigation = page.select('ul.topnav a[id]')[
                0].findParent().select('a')
        except:
    ...
    def catl3(self, page):
        ...
        try:
            ...
            navigation = page.select(
                'div.block-padding-lg')[0].select('div')[1].select('div a')
        except IndexError:
            # videa
        ...
        if page.select('a.prevChar'):
            ...
            item['type'] = 'prev'
            item['url'] = '#catl3#' + \
                self._url(page.select('a.prevChar')[0]['href'])
            result.append(item)
        ...
        if page.select('a.nextChar'):
            ...
            item['type'] = 'next'
            item['url'] = '#catl3#' + \
                self._url(page.select('a.nextChar')[0]['href'])
            result.append(item)
    ...
    def list(self, url):
        ...
        if url.find('#last#') == 0:
            ...
            raise Exception(
                "Invalid url, I do not know how to list it :" + url)
    ...
    def resolve(self, item, captcha_cb=None, select_cb=None):
        ...
        resolved = resolver.findstreams(
            url, ['(?P<url>http://www.youtube.com/watch\?v=' + m.group('vid') + ')'])
        result = []
        try:
            for i in resolved:
                item = self.video_item()
